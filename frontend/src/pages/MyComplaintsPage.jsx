...
import {
  ...
  MapPin, Tag, Building2, ArrowRight, ThumbsUp, ChevronDown,
  ChevronUp, RefreshCw,
} from 'lucide-react';
...
const STATUS_CONFIG = {
  pending:       { badge: 'bg-gray-100 text-gray-600',   label: 'Pending',     icon: Clock,         ring: 'ring-gray-200'   },
  'in-progress': { badge: 'bg-blue-100 text-blue-700',   label: 'In Progress', icon: RefreshCw,     ring: 'ring-blue-200'   },
  resolved:      { badge: 'bg-green-100 text-green-700', label: 'Resolved',    icon: CheckCircle2,  ring: 'ring-green-200'  },
  rejected:      { badge: 'bg-red-100 text-red-700',     label: 'Rejected',    icon: XCircle,       ring: 'ring-red-200'    },
};
    ...
    >
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          ...
          <span className="text-xs text-gray-400 flex-shrink-0">{timeAgo(complaint.createdAt)}</span>
        </div>

        {/* Title */}
        <h3 className="font-bold text-gray-900 text-sm mb-2 leading-snug">{complaint.title}</h3>
        ...
        {/* SLA bar */}
        {sla && complaint.status !== 'resolved' && complaint.status !== 'rejected' && (
          <div className="mt-4">
            ...
            <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full ${
                  ...
                  sla.daysLeft <= 2 ? 'bg-orange-500' : 'bg-gray-700'
                }`}
              ...
              <div className="mt-3 pt-3 border-t border-gray-100 space-y-2">
                ...
                {history.map((h, i) => (
                  <div key={i} className="flex items-start gap-2">
                    ...
                    <div>
                      <span className="text-xs text-gray-700">{h.message || h.status}</span>
                      {h.updatedAt && (
  ...
  return (
    <DashboardLayout>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard icon={RefreshCw}    label="In Progress"     value={inProgress} color="text-blue-600"  bg="bg-blue-50"   delay={0.15} />
                ...
                className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${
                  activeTab === tab
                    ? tab === 'Rejected'    ? 'bg-red-100 text-red-600' :
                      tab === 'In Progress' ? 'bg-blue-100 text-blue-600' :
                      tab === 'Resolved'    ? 'bg-teal-100 text-teal-600' :
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          {filtered.map((c, i) => (
            <ComplaintCard
              ...
              complaint={c}
              index={i}
              onView={(id) => navigate(`/complaints/${id}`, { state: { from: '/my-complaints', label: 'My Complaints' } })}
            />
          ))}
        </div>
      )}
