...
const PublicAnalyticsPage = () => {
  ...
  const generatePersonalReportPDF = async (filterType) => {
    ...
    try {
      const res = await complaintAPI.getAll({ mine: 'true', filter: filterType });
      ...
      doc.text('Somadhan Personal Activity Report', pageWidth / 2, 20, { align: 'center' });
      doc.setFontSize(10);
      doc.setTextColor(100);
      doc.text(`Citizen: ${user?.name} | Period: ${filterType.toUpperCase()}`, pageWidth / 2, 28, { align: 'center' });
