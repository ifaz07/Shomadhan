...
import {
  ...
  FileText, Users, Shield, ChevronDown, X, Sparkles
} from 'lucide-react';
  ...
  return (
    <div className="flex flex-col items-center">
      <svg width="96" height="96" viewBox="0 0 96 96">
        ...
        />
        <text x="48" y="52" textAnchor="middle" fontSize="14" fontWeight="bold" fill="#0f172a">{pct}%</text>
      </svg>
...
const ReportsPage = () => {
  ...
  // fetch complaints list for individual tab
  useEffect(() => {
  ...
  // fetch preview stats for summary tab
  const fetchPreview = async () => {
    ...
    } catch { toast.error('Failed to load preview stats'); }
    finally { setPreviewLoading(false); }
  };

  useEffect(() => { if (activeTab === 'summary') fetchPreview(); }, [activeTab, summaryType]);
  ...
  const downloadSummary = async () => {
    ...
    try {
      ...
      const fname = summaryType === 'monthly'
    ...
    finally { setSummaryDownloading(false); }
  };
    ...
    <DashboardLayout>
      <div className="max-w-5xl mx-auto space-y-6 px-1">
        ...
        >
          ...
          <h1 className="text-3xl font-black tracking-tight mb-2">PDF Report Generation</h1>
        ...
        {/* ── Tabs ── */}
        <div className="bg-slate-50 border border-slate-200 rounded-[1.5rem] p-2 flex gap-2">
          <button onClick={() => setActiveTab('summary')} className={tabCls('summary')}>
            <span className="flex items-center gap-2"><BarChart3 size={15} /> Summary Reports</span>
          </button>
          <button onClick={() => setActiveTab('individual')} className={tabCls('individual')}>
            <span className="flex items-center gap-2"><FileText size={15} /> Individual Case Report</span>
          </button>
        </div>
          ...
          {activeTab === 'summary' && (
            ...
            >
              ...
              <div className="bg-white rounded-[1.75rem] border border-slate-100 shadow-sm p-6">
                ...
                <button onClick={downloadSummary} disabled={summaryDownloading}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      <div className="bg-slate-50 rounded-2xl p-5">
                        ...
                        <div className="flex gap-6 justify-center flex-wrap">
                          <GaugeRing pct={resRate}
                            label="Resolution Rate" color="#10b981" />
                          <GaugeRing
                            pct={total > 0 ? Math.round((critical / total) * 100) : 0}
                            label="Critical Rate" color="#ef4444" />
                          <GaugeRing
              ...
              <div className="bg-white rounded-[1.75rem] border border-slate-100 shadow-sm p-6">
                ...
                <div ref={dropRef} className="relative">
                  <div className="flex items-center border border-slate-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-[#0d3b4b]/20">
                    ...
                    />
                    {searchTerm && (
                      <button onClick={() => { setSearchTerm(''); setSelectedComplaint(null); }}
                    ...
                    {dropdownOpen && (
                      ...
                      >
                        {complaintsListLoading ? (
                          ...
                          <button key={c._id} onClick={() => { setSelectedComplaint(c); setSearchTerm(c.ticketId); setDropdownOpen(false); }}
                    ...
                    >
                      ...
                      <div className="bg-[#0d3b4b]/5 rounded-xl p-3 mb-4">
                        ...
                        <div className="grid grid-cols-2 gap-1">
                          {['Case Overview','Reporter Information','Full Description','SLA & Deadline Tracking',
                            'Public Engagement','AI Analysis Results','Escalation Records','Status History Timeline','Evidence References'
                          ].map(it => (
                      ...
                      <button onClick={downloadComplaint} disabled={complaintLoading}
